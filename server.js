...
db.defaults({
  ...
  orders: [],
  nextUserId: 6,
  // Нумерация заявок/накладных продолжает старую (бумажную/1С) нумерацию,
  // а не начинается заново с 1 — см. scripts/set-next-order-id.js для
  // разового переноса счётчика на уже работающей базе (значение по
  // умолчанию ниже применяется только к СВЕЖЕЙ базе, где ключа ещё нет).
  nextOrderId: 22400,
  pushSubscriptions: []
}).write();
...
// Бэкфилл delivered_at для заявок, доставленных ДО появления этого поля
// (см. PUT /api/orders/:id/status) — точный момент доставки для них
// никто не записывал, но фото подписанной накладной (delivery_photo_at)
// снимается прямо перед закрытием заявки (обязательное условие перехода в
// "Доставлено"), так что это ближайшая доступная оценка. Заявки без фото
// (совсем старые, до появления самого delivery_photo_at) точного времени
// так и не получат — только дату (order.date), это уже ограничение
// исходных данных, а не то, что можно починить задним числом. Идемпотентно
// (при следующем запуске уже заполненные заявки просто пропускаются) —
// держим на старте, а не разовым скриптом в scripts/, чтобы сработало
// само при обычном деплое, без отдельной ручной команды.
(() => {
  let filled = 0;
  db.get('orders').value().forEach(o => {
    if (o.status === 'delivered' && !o.delivered_at && o.delivery_photo_at) {
      o.delivered_at = o.delivery_photo_at;
      filled++;
    }
  });
  if (filled > 0) {
    db.write();
    console.log(`✅ Заполнено время доставки (delivered_at) для ${filled} старых заявок (по фото накладной)`);
  }
})();
...
// Фото чека оплаты по QR — тот же смысл, что и фото наличности: подтверждает
...
app.post('/api/orders/:id/qr-photo', authMiddleware, (req, res) => {
  ...
  const fileName = `order${orderId}_qr_${Date.now()}.jpg`;
  fs.writeFileSync(path.join(WAYBILL_PHOTOS_DIR, fileName), buf);
  const url = '/uploads/waybill-photos/' + fileName;

  db.get('orders').find({ id: orderId }).assign({
    qr_photo: url,
    qr_photo_at: new Date().toISOString()
  }).write();
...
app.put('/api/orders/:id/status', authMiddleware, (req, res) => {
  ...
  if (status === 'delivered') {
    ...
    // Та же логика для QR: статус оплаты в приложении не подтверждает, что
    // деньги реально пришли на счёт — фото чека закрывает этот же разрыв,
    // что и фото наличности для нала (см. POST /api/orders/:id/qr-photo).
  ...
  const patch = { status };
  if (status === 'delivered') {
    // Момент фактической доставки — отдельно от created_at (оформление) и
    // in_transit_at (взятие в работу). Раньше нигде не фиксировался: в
    // карточке заявки было видно только выбранный слот (time_slot), а не
    // когда её реально довезли.
...
app.post('/api/orders/weights', authMiddleware, (req, res) => {
  ...
  entries.forEach(entry => {
    ...
    const items = typeof order.items === 'string' ? JSON.parse(order.items || '[]') : (order.items || []);
    // Если в заявке по ошибке (или намеренно, разными ценами) две строки с
    // одним кодом — искать первую попавшуюся нельзя: она может быть уже
    // подтверждена, и тогда КАЖДАЯ попытка взвесить вторую строку тихо
    // повторно бьёт в первую, а вторая так и остаётся неподтверждённой
    // навсегда, сколько раз склад ни сохраняй (см. заявку №22854, где именно
    // так и вышло). Сначала ищем ещё неподтверждённую строку с этим кодом —
    // именно её и должен закрыть новый ввод веса; если таких не осталось
    // (все уже подтверждены), берём первую — это и есть прежнее поведение
    // для правки уже подтверждённого веса (см. canOverride ниже).
    const item = items.find(it => it.code === code && !it.weight_confirmed) || items.find(it => it.code === code);
    if (!item) return;
    ...
    // против кг-остатка — это весь вводимый вес. При корректировке
    // (canOverride) qty уже в кг сам, и дельта — обычная разница со старым
    // значением.
    ...
    db.get('weighLog').push({
      ...
      // в истории как самое первое взвешивание, а не как правка с 0. Не
      // null означает "это исправление менеджера/админа поверх ошибки
      // склада" — видно в истории взвешивания, кто и что поправил.
  ...
  const orderDebts = orders
    ...
    .map(o => {
      ...
      return {
        ...
        // Точный момент доставки (см. delivered_at) — по просьбе владельца:
        // когда у одного должника несколько накладных на разные дни/часы,
        // одной только даты заявки в карточке не хватает, чтобы их не
        // путать. У продаж кассы (saleDebts ниже) доставки как таковой нет
        // (мгновенная продажа), поэтому поле только у заявок.
...
// Фиксирует, что оператор САМ ПОДТВЕРДИЛ отправку "Написать в WhatsApp" по
// конкретному долгу — фронт (см. sendReminder/waitForReturn в DebtsPanel)
// вызывает это только после того, как оператор вернулся с WhatsApp и
// ответил "Да" на прямой вопрос "отправили?", а не сразу по нажатию кнопки
// (раньше было так, путало операторов: открыл ещё не значит отправил).
// Само сообщение всё равно уходит вживую из приложения WhatsApp человека
// (см. toWhatsAppDigits) — это не техническое подтверждение доставки, а
// ответ оператора на слово чести, но уже не "нажал и всё", чтобы больше
// одного человека не писали одному и тому же должнику в один день вслепую
// (см. last_reminder_today
...
// (код, кол-во, дату, кто оформил), просто собираем их по коду в одном
// месте вместо отдельного отчёта на каждую заявку.
app.get('/api/products/:code/history', authMiddleware, (req, res) => {
...
app.post('/api/returns', authMiddleware, (req, res) => {
  ...
  // Возврат без заявки (свободный формат — товар с произвольным названием и
  // ценой от руки, не привязан ни к какому коду каталога) не имеет цены
  // продажи, к которой можно строго привязаться — значит и цену для него
  // фактически назначает тот, кто оформляет. Такой возврат может оформить
  // только admin; водителю/менеджеру доступен исключительно возврат по
  // конкретной заявке, где цена берётся из неё самой (см. ниже).
  if (!orderId && req.user.role !== 'admin') {
    return res.status(403).json({ error: 'Возврат без привязки к заявке может оформить только администратор' });
  }
  ...
  for (const it of (items || [])) {
    ...
    // Цену может свободно назначать только admin — водителю/менеджеру (уже
    // только в режиме "по заявке", см. проверку orderId выше) цена возврата
    // не берётся из запроса, а строго из самой заявки (то, что реально было
    // продано); фолбэк на каталог — на случай кода, которого в заявке нет.
