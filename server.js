...
app.post('/api/orders/weights', authMiddleware, (req, res) => {
  ...
  entries.forEach(entry => {
    ...
    // повторная правка запрещена — раньше вес можно было менять сколько
    // угодно раз вплоть до статуса "Доставлено", и уже подтверждённая
    // позиция молча перезаписывалась бы, разъезжаясь с тем, что уже
    // напечатано в накладной/загрузочном листе и что видел водитель.
    if (item.weight_confirmed) {
      errors.push(`"${item.name}" в заявке №${orderId}: вес уже сохранён и изменению не подлежит`);
      return;
    }
    ...
    // До подтверждения qty позиции — это короба (другая единица, другой
    // пул), поэтому в кг-пуле она ещё ничего не резервирует: дельта против
    // кг-остатка — это весь вводимый вес (item.weight_confirmed уже
    // проверено выше и здесь всегда false, повторная правка запрещена).
    const deltaKg = newWeight;
    ...
    db.get('weighLog').push({
      ...
      item_name: item.name,
      weight: newWeight,
      // Повторная правка запрещена (см. проверку item.weight_confirmed
      // выше) — каждая запись в этом логе теперь и есть первое и
      // единственное взвешивание позиции, prev_weight всегда пуст.
      prev_weight: null,
