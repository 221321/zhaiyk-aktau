...
// владельца) — заявки, продажи в кассе, смены, инкассации, возвраты,
// погашения долгов и история взвешивания зав. складом. Долги отдельной
// коллекцией не хранятся — они считаются от payment_debt в orders/sales
// (см. GET /api/debts на сервере), поэтому отдельно их зачищать не нужно:
// как только исчезнут заявки и продажи, исчезнут и долги.
//
...
// nextCashHandoverId, nextReturnId, nextWeighLogId) сбрасываются на 1 —
// новая нумерация начнётся заново (решение владельца: раньше выданные
// бумажные накладные с текущими номерами в расчёт не берём).
//
// ВАЖНО:
...
const TARGETS = [
  ...
  { collection: 'weighLog', nextIdKey: 'nextWeighLogId', label: 'История взвешивания' },
];
...
if (!process.argv.includes('--confirm')) {
  ...
  console.log('договорников и сотрудников.');
  console.log('');
...
console.log('Товары, цены, остатки, клиенты, договорники, сотрудники и пользователи');
console.log('не затронуты.');
